...
export function HeroCta({ variant = "light" }: { variant?: "light" | "dark" }) {
  ...
  return (
    <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
      <Link href="/studio" className={primaryCls}>
        立即体验
...
export function DetailActions({ product }: { product: Product }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <Link
        href={studioHref}
        className="w-full rounded-lg bg-primary-500 py-2.5 text-center text-sm font-medium text-white shadow-sm shadow-primary-500/25 transition hover:bg-primary-600"
      >
        立即体验
      </Link>
      <button
