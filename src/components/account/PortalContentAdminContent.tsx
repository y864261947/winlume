...
type CatalogModel = { model_name: string; vendor_key?: string; vendor_name?: string; vendor_logo?: string; portal_category?: Category; supported_endpoint_types?: string[] };
...
function categoryFromModel(model: CatalogModel): Category {
  ...
  const text = `${model.model_name} ${(model.supported_endpoint_types ?? []).join(" ")}`.toLowerCase();
  if (text.includes("image") || text.includes("dall") || text.includes("flux")) return "image";
  if (text.includes("video") || text.includes("sora") || text.includes("kling")) return "video";
  if (text.includes("audio") || text.includes("speech") || text.includes("whisper")) return "audio";
  if (text.includes("embed") || text.includes("rerank") || text.includes("rag")) return "embed";
...
function groupCatalog(models: CatalogModel[]): Vendor[] {
  ...
  for (const model of models) {
    const key = (model.vendor_key || model.vendor_name || "other").trim().toLowerCase().replace(/[^a-z0-9_-]/g, "-") || "other";
    const current = grouped.get(key) ?? { id: `catalog-${key}`, name: model.vendor_name || key, key, logoUrl: model.vendor_logo || "/vendors/other.svg", category: categoryFromModel(model), enabled: true, models: [] };
    current.models.push({ name: model.model_name, endpointTypes: model.supported_endpoint_types ?? ["chat"] });
...
async function readImage(event: ChangeEvent<HTMLInputElement>, onRead: (url: string) => void) {
  ...
  if (!file) return;
  if (!file.type.startsWith("image/") || file.size > 700_000) throw new Error("请上传 700KB 以内的图片文件，保证完整保存与快速加载。");
...
export default function PortalContentAdminContent() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [response, catalogResponse] = await Promise.all([fetch("/api/admin/portal-content", { credentials: "same-origin" }), fetch("/api/catalog/plaza", { credentials: "same-origin", cache: "no-store" })]);
      const body = await response.json() as PortalContent & { error?: string }; if (!response.ok) throw new Error(body.error || "加载失败");
      setContent({ carousel: body.carousel ?? [], notifications: body.notifications ?? [], modelVendors: body.modelVendors ?? [] });
      const catalog = await catalogResponse.json().catch(() => null) as { data?: CatalogModel[] } | null;
      setCatalogVendors(groupCatalog(catalog?.data ?? []));
    }
  ...
  function importCatalogVendors() {
    setContent((current) => {
      const configured = new Set(current.modelVendors.map((vendor) => vendor.key));
      const imported = catalogVendors.filter((vendor) => !configured.has(vendor.key));
      return { ...current, modelVendors: [...current.modelVendors, ...imported.map((vendor) => ({ ...vendor, id: uid("vendor") }))] };
    });
    setNotice("已将当前网关目录导入配置草稿；点击“保存并发布”后生效。");
  }
  ...
  return <ConsolePage eyebrow="平台" title="门户内容管理" description="管理首页轮播、站内通知，以及 API 模型的六类展示。保存后会发布到个人门户。" actions={<Button type="button" onClick={() => void save()} disabled={saving}>{saving ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{saving ? "正在发布" : "保存并发布"}</Button>}>
    {loading ? <p className="flex items-center gap-2 text-sm text-muted-foreground"><LoaderCircle className="h-4 w-4 animate-spin" />正在加载配置…</p> : <div className="grid gap-8">
      ...
      <section className="grid gap-3 rounded-xl border border-dashed border-primary-200 bg-primary-50/30 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">已同步 API 模型目录</h2><p className="text-sm text-muted-foreground">当前网关返回 {catalogVendors.reduce((total, vendor) => total + vendor.models.length, 0)} 个模型、{catalogVendors.length} 个厂商。可先查看，再导入为可编辑配置。</p></div><Button type="button" variant="outline" size="sm" disabled={!catalogVendors.length} onClick={importCatalogVendors}><Plus className="h-4 w-4" />导入当前目录</Button></div><div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{catalogVendors.map((vendor) => <details key={vendor.key} className="rounded-lg border border-border bg-background p-3"><summary className="cursor-pointer list-none"><div className="flex items-center gap-2"><img src={vendor.logoUrl} alt="" className="h-6 w-6 rounded object-contain" /><strong className="text-sm">{vendor.name}</strong><span className="ml-auto text-xs text-muted-foreground">{vendor.models.length} 个模型</span></div><span className="mt-1 block text-xs text-primary-600">{categories.find((item) => item.value === vendor.category)?.label}</span></summary><ul className="mt-3 grid gap-1 border-t border-border pt-2 text-xs text-muted-foreground">{vendor.models.map((model) => <li key={model.name}>{model.name}<span className="ml-1 text-[10px] text-muted-foreground">{model.endpointTypes.join(" · ")}</span></li>)}</ul></details>)}</div></section>
    </div>}
  </ConsolePage>;
}
