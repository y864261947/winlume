    ...
    <section className="mt-6 overflow-hidden rounded-xl border border-line bg-surface">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line px-4 py-3.5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-ink-900">模型广场</p>
            <p className="mt-0.5 text-xs text-ink-500">平台公开模型目录，所有访客均可浏览，无需登录。</p>
          </div>
        </div>
        <span className="rounded-full bg-primary-50 px-2.5 py-1 font-mono text-xs font-semibold text-primary-700 ring-1 ring-primary-100">
      ...
      {error ? (
        ...
        <div className="flex items-center gap-2 px-4 py-5 text-sm text-ink-500" role="status">
        ...
        <div className="grid gap-px bg-line sm:grid-cols-2 xl:grid-cols-3">
          {models.map((model) => (
            <article key={model.model_name} className="group bg-surface p-4 transition hover:bg-canvas">
              <div className="flex items-start justify-between gap-3">
                <p className="min-w-0 break-all font-mono text-sm font-semibold text-ink-900">
                  {model.model_name}
                </p>
                <Link
                  href={`/studio?model=${encodeURIComponent(model.model_name)}`}
                  title={`在 Studio 中使用 ${model.model_name}`}
                  aria-label={`在 Studio 中使用 ${model.model_name}`}
                  className="shrink-0 rounded-md p-1.5 text-ink-400 transition hover:bg-surface hover:text-primary-600"
                >
                  <ExternalLink className="h-3.5 w-3.5" />
                </Link>
              </div>
              <p className="mt-2 min-h-10 text-xs leading-5 text-ink-500">
                {vendors[model.vendor_id ?? 0] ?? "公开模型"} · {priceLabel(model)}
              </p>
              <div className="mt-3 flex flex-wrap gap-1">
                {(model.supported_endpoint_types ?? []).slice(0, 3).map((endpoint) => (
                  <span
                    key={endpoint}
                    className="rounded-md border border-line bg-surface px-1.5 py-0.5 text-[10px] text-ink-500"
                  >
                    {endpoint}
                  </span>
                ))}
