...
| Fastify gateway | `127.0.0.1:4010` | OpenAI-compatible public API, streaming proxying, API-key verification, wallet accounting. |
...
- Gateway directory: `/opt/winlume-gateway` production source checkout, port `127.0.0.1:4010`
...
The standalone web tarball deliberately does **not** bundle gateway source or
gateway runtime dependencies. Maintain a separate release checkout for it:

```bash
git clone <repository-url> /opt/winlume-gateway
cd /opt/winlume-gateway
npm ci
npm run db:migrate
```

The current gateway start script runs TypeScript with `tsx`, so this checkout
needs the full `npm ci` install rather than `npm ci --omit=dev`. Start it with a
separate unit:

```ini
[Unit]
Description=WinLume standalone protocol gateway
After=network-online.target
Wants=network-online.target
...
precedence. It does not update or restart `/opt/winlume-gateway`. Release the
gateway checkout and restart `winlume-gateway.service` as a coordinated step,
then verify both health endpoints. Keep database and gateway secrets
pre-provisioned on the host rather than passing them through workflow output.
